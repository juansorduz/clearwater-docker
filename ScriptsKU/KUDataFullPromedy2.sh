...
#NumberTest=33
Duration=1
testMainfolder=~/Kubernetes3
testfolder=~/Kubernetes3/$cps$duration
...
for i in ${DatosCPUastaire[@]}; do AverageCPUastaire=$(echo "$AverageCPUastaire + $i" | bc -l); echo CPU: $i; done
AverageCPUastaire=$(echo "scale=3;$AverageCPUastaire/$NumProms" | bc -l)
for i in ${DatosCPUastaire[@]}; do DiferenceVar=$(echo "$i - $AverageCPUastaire" | bc -l);DiferenceVar=$(echo "$DiferenceVar* $DiferenceVar" | bc -l); VarCPUastaire=$(echo "$VarCPUastaire + $DiferenceVar" | bc -l) ; echo Promedy: $AverageCPUastaire CPU: $i Dif Var: $DiferenceVar Var astaire: $VarCPUastaire; done
VarCPUastaire=$(echo "scale=3;$VarCPUastaire/$NumProms" | bc -l)
echo Average CPU astaire $AverageCPUastaire Var astaire $VarCPUastaire
