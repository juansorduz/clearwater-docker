...
mkdir -p ~/ClearwaterTestResults/Kubernetes2/$users$duration/$users$duration$NumTest
testfolder=~/ClearwaterTestResults/Kubernetes2/$users$duration/$users$duration$NumTest
export testfolder
...
while [ "$stateTest" -eq '1' ]; do
 ...
 #Regarding containers and their instances distribute the logs in eachone.
  ...
  for i in astaire_astaire cassandra_cassandra chronos_chronos bono_bono homer_homer homestead_homestead homestead-prov_homestead-prov ralf_ralf icscf_icscf scscf_scscf sipptest_sipptest; do
