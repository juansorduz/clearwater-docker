...
testfolder=~/ClearwaterTestResults/Kubernetes3/$users$duration/$users$duration$NumTest
export testfolder
...
. ~/clearwater-docker/ScriptsKU/KUTestSippStart3.sh $users $duration &
...
while [ "$stateTest" -eq '1' ]; do
 ...
 #Regarding containers and their instances distribute the logs in eachone.
  ...
  for i in urcassandra_urcassandra urhomestead_urhomestead ursprout_ursprout msccassandra_msccassandra mschomestead_mschomestead mscsprout_mscsprout bono_bono sipptest_sipptest; do
