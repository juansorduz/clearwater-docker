...
echo number of users: $users, duration: $duration, number of tests: $NumTest
...
mkdir -p ~/ClearwaterTestResults/Containers/$users$duration/$users$duration$NumTest
docker exec sipptest /usr/share/clearwater/bin/run_stress2 example.com $users $duration | tee ~/ClearwaterTestResults/Containers2/$users$duration/$users$duration$NumTestss/logsTestContainer.txt
