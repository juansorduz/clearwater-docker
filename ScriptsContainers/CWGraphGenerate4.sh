...
users="$1"
duration="$2"
testfolder=~/ClearwaterTestResults/Containers4/$users$duration
...
sed -i '1i icscf.sprout          icscf.sprout           icscf.sprout' $testfolder/icscf.sprout.csv
sed -i '1i i_c_s_c_f          i_c_s_c_f           i_c_s_c_f' $testfolder/i_c_s_c_f.csv
sed -i '1i scscf.sprout          scscf.sprout           scscf.sprout' $testfolder/scscf.sprout.csv
sed -i '1i s_c_s_c_f          s_c_s_c_f           s_c_s_c_f' $testfolder/s_c_s_c_f.csv
...
for FILE in astaire cassandra chronos bono homer homestead ralf icscf.sprout scscf.sprout i_c_s_c_f s_c_s_c_f sipptest; do
    gnuplot <<- EOF
...
gnuplot <<- EOF
    ...
    plot "$testfolder/scscf.sprout.csv" using 0:2 with lines lt rgb "#5F9EA0", "$testfolder/icscf.sprout.csv" using 0:2 with lines lt rgb "#00FFFF", "$testfolder/i_c_s_c_f.csv" using 0:2 with lines lt rgb "#7FFFD4", "$testfolder/s_c_s_c_f.csv" using 0:2 with lines lt rgb "#0000FF", "$testfolder/cassandra.csv" using 0:2 with lines lt rgb "#8A2BE2", "$testfolder/sipptest.csv" using 0:2 with lines lt rgb "#FFD700", "$testfolder/homestead.csv" using 0:2 with lines lt rgb "#006400"
EOF
