...
for i in astaire urcassandra msccassandra chronos bono ellis homestead-prov homer urhomestead mschomestead ralf ursprout mscsprout sipptest; do
   ...
   [ -e $testfolder/PromediosLatencia$i$cps ] && rm $testfolder/PromediosLatencia$i$cps
done
while [ $NumTest -lt $NumberTest ]; do
  for i in astaire urcassandra msccassandra chronos bono ellis homestead-prov homer urhomestead mschomestead ralf ursprout mscsprout sipptest; do
    tail -n +20 "$testfolder/$cps$duration$NumTest/$i.csv" > "$testfolder/$cps$duration$NumTest/$i"
    head -n +20 "$testfolder/$cps$duration$NumTest/$i" > "$testfolder/$cps$duration$NumTest/$i"
    SumCPU=0
    SumRAM=0
    PromCPU=0
    PromRAM=0
    ...
    done < "$testfolder/$cps$duration$NumTest/$i"
    #echo  $NumData

    #romRAM=$(echo "scale=3; $SumRAM/$NumData" | bc -l)
    if [ $NumData = '0' ];
    then
      echo CPM $cps Prueba $NumTest con falla, no se considerara en los promedios generales.
    else
        PromCPU=$(echo "scale=3; $SumCPU/$NumData" | bc -l)
        PromRAM=$(echo "scale=3; $SumRAM/$NumData" | bc -l)
        echo $PromCPU >> $testfolder/PromediosCPU$i$cps
