...
mkdir -p ~/ClearwaterTestResults/Containers4/$users$duration
...
. ~/clearwater-docker/ScriptsContainers/CWPrueba4.sh $users $duration &
...
#Regarding containers and their instances distribute the logs in eachone.
 ...
 for i in astaire cassandra chronos bono homer homestead ralf icscf.sprout scscf.sprout i_c_s_c_f s_c_s_c_f sipptest; do
   # if [ "$i" -eq 'sprout' ]; then
